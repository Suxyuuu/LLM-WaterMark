...
class PEFTArguments:
    ...
    num_virtual_tokens: int = field(default=64)  # Used for prompt tuning, prefix tuning and p-tuning
    mapping_hidden_dim: int = field(default=1024)
    # token_dim: int = field(default=768)
    num_transformer_submodules: int = field(default=12)
    num_attenion_heads: int = field(default=12)
    num_layers: int = field(default=12)

def get_peft_config(peft_args: PEFTArguments):
    # if peft_args.peft_mode == "bitfit":
    #     # bitfit
    ...
    #     print('param requires_grad down!')
    if peft_args.peft_mode == "lora":
        peft_config = LoraConfig(
            task_type=TaskType.CAUSAL_LM, 
            inference_mode=peft_args.inference_mode,
        ...
        peft_config = PrefixTuningConfig(
            ...
            # token_dim=peft_args.token_dim,
            num_transformer_submodules=peft_args.num_transformer_submodules,
            num_attention_heads=peft_args.num_attenion_heads,
            num_layers=peft_args.num_layers,
...
def main():
    ...
    print("Setup PEFT")
    print("finetune mode: ",peft_args.peft_mode)
    ...
    if training_args.do_eval:
        ...
        def compute_metrics(eval_preds):
            ...
            print(f"Initial preds shape: {preds.shape}")
    ...
    if training_args.do_train:
        ...
        torch.save(model.state_dict(), f"{training_args.output_dir}/{peft_args.peft_mode}_adapter_model.bin")
        #############################################################
